...
class OrderController extends BaseController
{
    ...
    public function updateOrderid()
    {
        ...
        $order = D("Order")->get($condition, true, "id desc");
        if ($order) {
            D("Order")->save(array("id" => $order["id"], "status" => 1));
            $this->ajaxReturn(array("info" => "success", "msg" => "操作成功", "status" => 1));
        } else {
            $this->ajaxReturn(array("info" => "error", "msg" => "没有此订单", "status" => 0));
        }
    }
    ...
    public function getOrderList()
    {
        ...
        if ($order) {
            ...
            $this->ajaxReturn(array("info" => "error", "msg" => "没有订单", "status" => 0));
        }
    }
}